<x-app-layout>
    ...
    <form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <h2 class="text-2xl font-semibold mb-4 text-white">Edit Data {{ $product->nama_barang }}</h2>

        <label class="block mb-3">
            <span class="text-sm text-zinc-300">Nama Barang</span>
            <input type="text" name='nama_barang' placeholder="Masukkan nama barang" required value="{{ $product->nama_barang }}"
        ...
        <label class="block mb-3">
            ...
            <input type="number" min="0" value="{{ $product->jumlah_barang }}" name="jumlah_barang" placeholder="Masukkan jumlah barang" required
                class="mt-1 block w-full rounded-lg bg-transparent border border-zinc-700 px-3 py-2 text-white placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-600" />
        </label>
