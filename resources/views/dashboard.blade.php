<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto w-full">
        <div
            class="bg-white dark:bg-gradient-to-tr from-zinc-950 to-zinc-900 overflow-hidden border border-zinc-800 rounded-lg">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('product.create') }}"
            class="bg-gradient-to-tr from-zinc-900 to-zinc-800 px-4 py-2 text-white border border-zinc-800 rounded-md hover:brightness-110">
            + Tambah Barang
        </a>
    </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-white">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-zinc-800 dark:text-zinc-300">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Nama Barang</th>
                                <th scope="col" class="px-4 py-3 text-center">Jumlah</th>
                                <th scope="col" class="px-4 py-3 text-center">Tanggal Peminjaman</th>
                                <th scope="col" class="px-4 py-3 text-center">Tanggal Pengembalian</th>
                        ...
                        <tbody>
                            @foreach ($form as $f)
                                <tr class="border-b dark:border-zinc-800">
                                    ...
                                    <td class="px-4 py-3 text-center">{{ $f['jumlah_barang'] }}</td>
                                    <td class="px-4 py-3 text-center">{{ $f['tanggal_pinjam'] }}</td>
                                    <td class="px-4 py-3 text-center">{{ $f['tanggal_kembali'] }}</td>
                                    <td class="px-4 py-3">

                                        <div class="flex items-center justify-center gap-2">
                                            <a href="#"
                                                class="block px-4 py-2 hover:bg-emerald-500 dark:bg-gradient-to-bl from-emerald-600 to-green-950 dark:text-zinc-300 dark:hover:text-white border border-zinc-800 rounded-md">
                                                Approve
                                            </a>

                                            <form action="{{ route('form.destroy', $f->id) }}" method="POST"
                                                onsubmit="return confirm('Yakin menolak permintaan ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="w-full block bg-red-600 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-red-700 dark:text-zinc-300 dark:hover:text-white rounded-md">
